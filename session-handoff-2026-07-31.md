...
**⚠ What is NOT proven: the new behaviour.** No real message has exercised the `[ACTIVE]` / `[UNDER CONTRACT - not available]` tagging. It also **cannot be seen with today's data** — the dev database is 4 ACTIVE / 3 SOLD, so nothing is tagged under-contract and a digest looks identical to before. The cheapest real check is two turns: a `listing-status-update` "under contract" on one listing, then a digest to confirm it is excluded. It costs credit, so it is the owner's call and was deliberately not done.
...
- **⚠ The new behaviour is NOT exercised.** No real message has hit the `[ACTIVE]` / `[UNDER CONTRACT - not available]` tagging, and with today's data (4 ACTIVE / 3 SOLD) it is invisible — nothing is under contract, so a digest looks unchanged. Two turns would close it: a `listing-status-update` "under contract", then a digest confirming that listing is excluded. Costs credit; owner's call.
...
>
> **One thing genuinely unfinished, and it is cheap:** PR #92's `[ACTIVE]` / `[UNDER CONTRACT - not available]` tagging is deployed but **has never been exercised by a real message**, and it is invisible with today's data (4 ACTIVE / 3 SOLD — nothing is under contract). Two turns would close it: a `listing-status-update` "under contract" on one listing, then a digest confirming that listing is excluded from the roundup. It costs credit, so ask me first.
>
