...
| Skill / plugin versions | **skills bumped 2026-07-30 (latest) and PENDING a gateway restart** — `listing-to-social` 0.5.0, `listing-status-update` **0.5.1**, `just-sold` **0.3.1**, `listing-reengagement` **0.2.1**, `weekly-digest` 0.1.0, `buyer-inquiry` 0.4.0. No copy needed (external_dirs → working tree); a restart or `/new` is. The parity row above is about **plugins only** and cannot see this. Plugins `listing-footer-reminder` 1.5, `buyer-listings-context` 1.3, `active-listings-context` 1.1, both sync plugins 1.0 |
...
**⚠ ONE THING IS PENDING DEPLOYMENT, and it is the only thing:** `just-sold` 0.3.1, `listing-status-update` 0.5.1, `listing-reengagement` 0.2.1.

- **Skills do NOT deploy like plugins — no copy is needed.** The operator profile's `skills.external_dirs` is `C:/dev/Portfolio/AI-Marketing/agent/skills`, read from the live `config.yaml`: it loads them straight out of the working tree. They need only **`hermes -p autoestate gateway restart`, run by you from your own shell** (an agent session doing it has caused a full outage before), or a `/new` session — a long-lived session never reloads a `SKILL.md`.
- **The plugin-parity recipe cannot see this** and will keep saying "0 drift, nothing pending", which is true of plugins and irrelevant here. Verify skills by resolving them (`_find_all_skills`, CLAUDE.md §5), not by parity.
...
>
> **⚠ One thing is pending deployment, and it is the only thing: three skills.** `just-sold` 0.3.1, `listing-status-update` 0.5.1, `listing-reengagement` 0.2.1. **Skills deploy unlike plugins — no copy.** The operator profile's `skills.external_dirs` points at this working tree, so they only need `hermes -p autoestate gateway restart` **run by me from my own shell** (an agent doing it has caused an outage), or a `/new` session. **The plugin-parity recipe will report "0 drift, nothing pending" and be wrong about this** — it only compares plugins. And the sharp edge: because skills load from the working tree, **a branch checkout changes what the live agent loads**, so check which branch is out before trusting any live skill test.
>
