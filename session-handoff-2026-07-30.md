...
- **Those two engineering items are DONE** (2026-07-30, latest), settled together as recommended, across **three** `SKILL.md` files rather than the two the docs predicted — `listing-reengagement` carried the same wording and was found by grepping. The footer-length contradiction is fully fixed; the `UNDER_CONTRACT` item took option (b), which **removes the false statement but leaves the lookup gap open** — option (a) is still the permanent fix and still unstarted. **This is the only thing in the repo pending deployment**, and skills deploy unlike plugins: no copy, just a gateway restart. See "What to do next".
...
| | State |
|---|---|
| `autoestate` gateway (operator) | **running, PID 35040**, created 18:04:54 on 2026-07-30 (was 31076; restarted by the owner to load PR #86's skill changes), `Gateway running with 2 platform(s)` — Telegram + WhatsApp both connected, log clean |
...
| WhatsApp bridges | operator bridge **up on port 3000, PID 31116** — *unchanged across **two** gateway restarts now* (07-29 late, and 07-30 for PR #86), which is the documented "reuse the bridge if it is healthy AND its `scriptHash` matches" branch firing rather than kill-and-replace. Don't rely on it — the failing branch is what caused a real outage. Zero `405`s since recovery. Port **3001** free (buyer gateway stopped) |
...
| Skill / plugin versions | **skills bumped 2026-07-30 (latest) in PR #86 and DEPLOYED** (restart verified by start time vs mtime) — `listing-to-social` 0.5.0, `listing-status-update` **0.5.1**, `just-sold` **0.3.1**, `listing-reengagement` **0.2.1**, `weekly-digest` 0.1.0, `buyer-inquiry` 0.4.0. No copy needed (external_dirs → working tree); a restart or `/new` is. The parity row above is about **plugins only** and cannot see this. Plugins `listing-footer-reminder` 1.5, `buyer-listings-context` 1.3, `active-listings-context` 1.1, both sync plugins 1.0 |
...
~~**There is now NO unblocked work left.**~~ ~~**True on 2026-07-29, no longer true after the two 07-30 sweeps** — two unblocked engineering items.~~ **Both are now DONE (2026-07-30, latest), in one change as recommended.** Everything that gates a *pilot* is account-level again — Hetzner + `terraform apply`, the Vercel deploy, the Cloudflare + R2 bucket.

**✅ MERGED (PR #86) AND DEPLOYED — nothing is pending deployment.** `just-sold` 0.3.1, `listing-status-update` 0.5.1, `listing-reengagement` 0.2.1.

- **Skills do NOT deploy like plugins — no copy was needed.** The operator profile's `skills.external_dirs` is `C:/dev/Portfolio/AI-Marketing/agent/skills`, read from the live `config.yaml`: it loads them straight out of the working tree. Only a restart was required — the owner ran `hermes -p autoestate gateway restart` from their own shell (an agent session doing it has caused a full outage before).
- **Verified, and the start-time argument is the proof:** PID **31076 → 35040**, created **18:04:54**, versus skill-file mtimes of **18:03:08** — the process began after the files, ruling out a cached load. `Gateway running with 2 platform(s)`, both connected, no errors. **Not exercised with a real message** — the deployment is proven, the new wording's behaviour is not.
- **The plugin-parity recipe cannot see any of this** and will keep saying "0 drift, nothing pending" — true of plugins, irrelevant to skills. Verify skills by resolving them (`_find_all_skills`, CLAUDE.md §5) and by start time vs mtime, not by parity.
- **Sharp edge worth knowing before you test anything live: a branch checkout changes what the live agent loads.** These edits were on disk for the running gateway before review or merge, and checking out an older branch — or `git stash` — silently downgrades the live agent's skills on its next session. Check which branch is out before trusting a live skill test.
- **What was fixed, precisely, because half-fixes have read as whole ones here twice:** the footer-length contradiction is *fully* fixed; the `UNDER_CONTRACT` change removes the **false statement** ("that listing isn't on record") but **does not close the gap** — an under-contract listing still can't be found by name, so option (a), widening `/api/listings/active`, is still the permanent fix and is still unstarted. Costed in TODO's known issues; still best folded into item 12b's redeploy.
...
>
> **What's left splits in two.** Everything that gates a pilot is **account-level and yours**: Hetzner + `terraform apply` (TODO 3), the Vercel deploy (TODO 4 — land the partial unique index on `Listing` first), and the Cloudflare + R2 bucket (TODO 13, smallest and independent). **Don't start any of them without me** — they cost money and none of the accounts exist. The two engineering items the 07-30 sweeps left open are **done** (one change, three `SKILL.md` files) — but note the `UNDER_CONTRACT` one took the wording-only option, so it removed a false statement and did **not** close the gap; option (a) is still unstarted.
>
> **Nothing is pending deployment.** PR #86's three skills (`just-sold` 0.3.1, `listing-status-update` 0.5.1, `listing-reengagement` 0.2.1) are merged and live — I restarted the gateway myself and it was verified by start time vs file mtime, not by assuming. **But learn the mechanism, because only the plugin half was ever written down: skills deploy unlike plugins — no copy.** `skills.external_dirs` points at this working tree, so a restart is the whole deployment. **The plugin-parity recipe will report "0 drift, nothing pending" and be silent about skills entirely** — it only compares plugins. And the sharp edge: because skills load from the working tree, **a branch checkout, or `git stash`, changes what the live agent loads**, so check which branch is out before trusting any live skill test.
>
...
>
> **On sweeps:** don't run a fourth `/inspect` — the next belongs after Hetzner/Vercel, when real infrastructure exists to inspect against. On `/docs`, the rule was revised on 07-30 by overruling it: **a pass that re-runs the live checks earns its place; a pass that only re-reads the prose does not.** All four of that pass's findings came from running things, none from reading. **That lesson is now baked into the `/docs` and `/fastpassdocs` command files themselves** (user-level, `%USERPROFILE%\.claude\Commands\` — outside this repo, so git will never show it; see CLAUDE.md), along with when and how to use subagents. So you inherit it automatically rather than needing to re-derive it — overrule it deliberately if you disagree.
>
