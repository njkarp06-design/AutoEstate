...
LISTING_RECORD_REMINDER = (
    ...
    "confirmed identity first. Write the post content as normal, then in "
    "place of the footer end with a clearly-marked confirmation question - "
    "not a soft sign-off line. It must stand out as a real question the "
    "sender has to answer, separated from the post, framed around updating "
    "their records (not the post, which is already done), naming the "
    "listing's identity facts so a wrong match is obvious, and telling them "
    "exactly how to confirm. Use this shape:\n"
    "\"---\\n⚠️ Before I mark this in your records - is this the "
    "<area> listing (<rooms> rooms, <sqm> sqm)? Reply *yes* to confirm.\"\n"
    "Append the footer only in the follow-up turn after the sender "
    "confirms. This exception never applies to a NEW listing "
    "(listing-to-social) - its facts are stated in the message itself, so "
    "its footer is always appended immediately.\n\n"
