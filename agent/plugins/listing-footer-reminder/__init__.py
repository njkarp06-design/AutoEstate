...
LISTING_RECORD_REMINDER = (
    ...
    "confirmed identity first. START your reply with a clearly-marked "
    "confirmation question - the very first thing in the message, before "
    "the post content, so it can't be missed - not a soft sign-off buried "
    "at the end. It must read as a real question the sender has to answer, "
    "framed around updating their records (not the post, which is already "
    "done below it), naming the listing's identity facts so a wrong match "
    "is obvious, and telling them exactly how to confirm. Then a divider, "
    "then the post content as normal. Use this shape:\n"
    "\"⚠️ Before I mark this in your records - is this the <area> listing "
    "(<rooms> rooms, <sqm> sqm)? Reply *yes* to confirm.\\n---\\n"
