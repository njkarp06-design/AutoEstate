...
- **Buyer bot can now answer amenity questions** (2026-07-28). An optional `Features:` line on the Listing Record footer carries parking/elevator/balcony/etc. through to the buyer instance. Verified live both ways: it answers a listed feature and **defers** on an unlisted one rather than inferring absence. **Deployed and confirmed live 2026-07-28**: a real listing carried `Features: elevator, balcony, renovated kitchen`, updated the existing Rothschild row in place (no duplicate), and the buyer bot then answered "does it have a balcony?" while still deferring on "is there parking?" — verified in `api_content`, not inferred from the replies. Known gap, accepted: proximity facts ("5 min walk to the beach") aren't captured, see TODO.
