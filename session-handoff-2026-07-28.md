...
- **12a — operator channel to Telegram. DECIDED, BUILT and LIVE-TESTED 2026-07-28: both channels, drop WhatsApp later.** `listing-to-social` now proven end-to-end over Telegram — the first real listing that channel has ever carried (`REF-QTPT4`, Shenkin). The footer-reminder plugin was confirmed firing there via `api_content`. The other four skills stay unverified on Telegram. The dev profile already ran Telegram on a **live** token (`@autoestate_test_bot`, verified via `getMe`); the real gap was that the **Terraform template shipped no Telegram at all**, so customers would have been WhatsApp-only. Now wired alongside WhatsApp — empty token means no adapter starts, and `variables.tf` names exactly what to delete when WhatsApp goes. Rendered in `terraform console`, `fmt`/`validate` clean, **never applied**. Left to do: rename the bot in BotFather (it is still the "Test Bot"), and live-test the outbound flow over Telegram — the DB has **0** Telegram runs, so that channel has never carried a real listing.
- **12d — per-listing ref codes. BUILT, MERGED (PR #59), DEPLOYED and LIVE-TESTED 2026-07-28.** A real tapped ad link resolved `REF-V42TS` straight to Rothschild with no disambiguation question, deferred on an unlisted feature and answered a listed one; `api_content` confirmed the injected rule really reached the model. **The live test found one defect the feature itself introduced** — a buyer arriving by ad link never types an area name, so the lead failed to link to its listing. Fixed (codes checked first in `/api/inquiries`) and proven by replaying the real turn. The buyer gateway was stopped again afterwards.
