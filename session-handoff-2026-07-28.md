...
- **⚠ Something IS now half-deployed.** The dual-channel change edits the buyer `config.yaml` and **all five** plugins in the repo only. Merging deploys nothing — plugins and profile configs are physical copies inside each profile. Before the buyer instance next starts, copy them across and re-run CLAUDE.md §5's parity recipe. The **operator** profile's three plugins changed too, so that gateway needs a restart **by the owner, from their own shell** (an agent session has caused an outage before). Until then, repo↔live parity will correctly report drift.
