...
- **Every roadmap feature is built, live-tested and merged.** Every step that **gates a pilot** is account-level and belongs to the owner. Two things qualify the old "everything remaining is productionization" line, both from the 07-30 sweep: its 7 fixes are on an **unmerged branch**, and it left **one engineering item** needing a decision (see below).
...
- **A third `/inspect` sweep ran on 2026-07-30** — the pre-deploy code pass this file recommended, and it was worth it: **8 findings, 1 critical.** Seven fixed on branch `fix/inspect-sweep`, one deferred with its options costed. That branch was **pushed to `origin`; no PR was opened and nothing was merged** — so `main` does not carry these fixes. Don't trust that sentence either: `gh pr list --state all` and `git log origin/main..origin/fix/inspect-sweep` are the answer, and this line is exactly the kind that goes stale on the next merge. The critical finding is that the Terraform secret-injection guard against wiping a customer's `.env` had never worked. **No plugin, skill or live profile config changed, so nothing needed redeploying and the parity lines below still hold.** See the section at the end of this file.
