...
2. **The four skills never tested over Telegram** (status-update, just-sold, re-engagement, digest). `listing-to-social` is proven there twice. Cheap, and genuinely unverified — but nothing is broken. **This now needs the owner in the loop**: it means sending real messages from your own Telegram account and spends Anthropic credit. An agent session can drive the three-layer verification (gateway log → `state.db` → dev database) but cannot send the messages.
3. **Create the Telegram notifier bot** and set `OPERATOR_TELEGRAM_BOT_TOKEN` so operator lead alerts push. The dashboard records leads either way. **Also owner-only** — it means talking to BotFather from your Telegram account. Once the token exists it is a one-line `.env` addition on the reporting app.

**So nothing substantial is left that an agent session can finish alone.** Both remaining unblocked items need the owner's own Telegram account, and everything else is account-level (Hetzner / Vercel / Cloudflare).
...
**Later the same day (reporting-app source only — no live system touched, nothing redeployed):**
