...
- **The one new defect is FIXED (later the same day).** Hermes background-review turns are no longer ingested — filtered at `/api/ingest` and `/api/inquiries`, and the 3 affected historical rows deleted (29 → 26 runs). Reporting-app source only, so nothing needed redeploying. Details in TODO's known-issues list and CLAUDE.md.
...
| Repo↔live plugin parity | was **5/5** that evening — **expect 3/5 once the timeout PR merges**, since `active-listings-context` (→ 1.1) and `buyer-listings-context` (→ 1.3) are changed in the repo and not yet copied into the profiles. That drift is the pending deploy, not corruption. Re-run the recipe |
...
3. **Create the Telegram notifier bot** and set `OPERATOR_TELEGRAM_BOT_TOKEN` so operator lead alerts push. The dashboard records leads either way. **Owner-only** — it means talking to BotFather from your Telegram account. Three things to get right, all already checked against the code: the token is read at **module scope** in `lib/notify-operator.ts`, so the 4127 dev server must be **restarted** after setting it; `Customer.operatorTelegramChatId` is currently **null**, so set it on the Settings page or leads fall back to the `OPERATOR_TELEGRAM_CHAT_ID` env var; and you must **`/start` the new bot** before it can DM you. Also `/setjoingroups` → Disable — a fresh bot defaults to group-join **enabled**.

**⚠ 4. DEPLOY THE PENDING PLUGIN FIX.** `TIMEOUT_SECONDS` 3 → 10 in `active-listings-context` and `buyer-listings-context`. Merging deploys nothing — copy both into their profiles' `plugins/` dirs and restart the **operator** gateway from your own shell (the buyer gateway is stopped and takes it on next start), then re-run the parity recipe. Until then the parity check will legitimately report drift on those two.

**So the only unblocked item left needing no account is #3**, and it still needs your Telegram. Everything else is account-level (Hetzner / Vercel / Cloudflare) or the deploy step above.
