...
- [~] 1H stand up dev buyer instance + end-to-end test. **Instance BUILT and statically verified 2026-07-26; live run not yet done** (needs the Telegram bot token, below). Sub-status:
  ...
  - [ ] **BLOCKED on user:** create a dedicated dev Telegram bot in @BotFather and supply the token → fill `TELEGRAM_BOT_TOKEN` in the profile `.env`.
  - [ ] Start `hermes -p autoestate-buyer gateway` and run the end-to-end buyer→dashboard test.
