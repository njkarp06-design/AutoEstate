...
**Status:** every planned feature is built, live-tested and merged — the last, buyer-inquiry, in PR #34. The `/inspect` forensic sweep (PR #37) then fixed 41 defects across infra, plugins and the reporting app. Those fixes were then **deployed to the live profiles and the operator gateway restarted (2026-07-27)** — merging alone would not have done it, since plugins are physical copies inside each profile. **Nothing is blocked on more building.** Everything below is productionization. *(One deployment step is outstanding as of 2026-07-28: PR #52's files are deployed, but the operator gateway still needs its restart — see the deployment note below.)*
...
**Deployed 2026-07-28, one step outstanding.** PR #52's five plugin files and the buyer `config.yaml` were copied into the live profiles and verified (parity 5/5; buyer config parity clean with the documented exclusions; live-profile resolution confirms `whatsapp` yields the same 3 tools as `telegram`, gating on, stranger denied `/profile`, `bridge_port` 3001). The buyer gateway was stopped throughout, which is when editing its config is safe, and its machine-written `onboarding.seen` was preserved rather than clobbered.
**Still outstanding: the operator gateway restart.** Three of its plugins changed, so PID 36868 is executing pre-#52 code until the owner runs `hermes -p autoestate gateway restart` **from their own shell** — restarting it from an agent session has caused a full outage before.

**Plugin deployment: verified 2026-07-28 (post-#52).** Parity is 5/5 with the repo, split by role — the operator profile carries `sync-to-webapp`, `listing-footer-reminder`, `active-listings-context`; the buyer profile carries `buyer-listings-context`, `sync-inquiries-to-webapp`. The operator gateway (PID 36868) started at 19:58:34, five minutes *after* the newest plugin file was written at 19:53, which is what proves it loaded new code rather than a cache; its log shows `Gateway running with 2 platform(s)`, no errors, and the inert-plugin warning correctly silent. **The buyer profile's copies are in place but unloaded** — its gateway is stopped and its token revoked, so they take effect whenever it next starts. Re-run CLAUDE.md §5's parity recipe rather than trusting this paragraph; the next plugin change invalidates it.
...
- [ ] Mint a fresh BotFather token and put it in the buyer profile's `.env`. Telegram is **not** being skipped — the transport decision (2026-07-28) kept it as the Facebook-sourced buyer channel.
