...
**In flight:** nothing. Every PR opened to date is merged and its branch deleted; no PR is open. (Not enumerated — a PR range is stale the moment the next one merges. `gh pr list` is the source of truth.)
