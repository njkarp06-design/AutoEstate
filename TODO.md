...
**⚠ NOT YET DEPLOYED (2026-07-28):** the dual-channel change edits the buyer profile's `config.yaml` and all five plugins in the **repo only**. Merging deploys nothing — plugins and profile configs are physical copies inside each profile. Before the buyer instance next starts: copy the five plugin `__init__.py` files and the buyer `config.yaml` into the live profiles and re-run the parity recipe. The buyer gateway is stopped (and its token revoked), which is exactly when editing its `config.yaml` is safe; the **operator** profile's three plugins also changed, so that gateway needs a restart **by the owner, from their own shell**.

**Plugin deployment: as of 2026-07-27 (now stale — see the deployment warning above).** Parity is 5/5 with the repo, split by role — the operator profile carries `sync-to-webapp`, `listing-footer-reminder`, `active-listings-context`; the buyer profile carries `buyer-listings-context`, `sync-inquiries-to-webapp`. The operator gateway (PID 36868) started at 19:58:34, five minutes *after* the newest plugin file was written at 19:53, which is what proves it loaded new code rather than a cache; its log shows `Gateway running with 2 platform(s)`, no errors, and the inert-plugin warning correctly silent. **The buyer profile's copies are in place but unloaded** — its gateway is stopped and its token revoked, so they take effect whenever it next starts. Re-run CLAUDE.md §5's parity recipe rather than trusting this paragraph; the next plugin change invalidates it.
