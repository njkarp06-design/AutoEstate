...
- [ ] **Deploy after merge:** copy `buyer-listings-context/__init__.py` into the live `autoestate-buyer` profile and re-run the parity recipe. Merging deploys nothing. The buyer gateway is stopped by design, so it picks the change up on its next start — no restart-by-owner needed *unless* it is running.
