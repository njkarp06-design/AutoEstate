...
Last updated: 2026-07-25 (buyer-inquiry 1A–1F built + verified; 1G seam built, transport pending decision; 1H blocked on Telegram token).
...
- [~] **1G operator notification — seam built 2026-07-25, TRANSPORT PENDING USER DECISION.** `lib/notify-operator.ts` fires best-effort from `/api/inquiries` on a deferring reply, with the buyer's contact attached. Wired for **Telegram** (recommended: free, no domain/deliverability) but **inert until `OPERATOR_TELEGRAM_BOT_TOKEN` + `OPERATOR_TELEGRAM_CHAT_ID` are set** — nothing sent to any external service without the operator's own creds. Env-based config is single-operator (dev/MVP); multi-tenant prod needs per-customer routing (a `Customer.operatorTelegramChatId` column) — NOT added without sign-off (schema/account commitment). **Decide transport before the live 1H test.**
