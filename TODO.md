...
**⚠ NOT YET DEPLOYED (listing features, 2026-07-28).** Two plugins changed — `listing-footer-reminder` (1.5) and `buyer-listings-context` (1.1) — plus a schema migration. **Order matters:** the migration must be applied BEFORE the plugins are deployed. Reversed, footers carry a `Features:` line while the column doesn't exist, Prisma throws inside ingest's blanket `try/catch`, and listings silently stop being tracked while every reply still looks perfect. The dev database is already migrated; a future Vercel deploy must follow the same order. Then copy both plugins into their profiles, re-run the parity recipe, and the **owner** restarts both gateways.
