...
- [~] 1H stand up dev buyer instance + end-to-end test. **Instance BUILT and statically verified 2026-07-26; live run not yet done** (needs the Telegram bot token, below). Sub-status:
  ...
  - [~] **End-to-end test — operator-account leg DONE 2026-07-26, flatmate leg still outstanding.** Seven real messages sent to `@autoestate_buyerdev_bot`. **Skill layer passed on every case it owns** (price from injected data only; SOLD reported honestly; Hebrew mirrored; missing parking fact deferred not invented; viewing deferred + contact requested; prompt injection refused in role; defer sentence verbatim ×3). Synced as one Inquiry, 14 messages, correct order, `source: telegram`, `status: NEEDS_OPERATOR`. Four surrounding defects found and fixed — see CLAUDE.md for detail:
    ...
    - [ ] **Ambiguity path still unverified** — the "is it still available?" turn had a clear referent from the prior message, so the model resolved it rather than asking. Not a failure; the case wasn't cleanly exercised. Needs a **fresh session from the flatmate's account**.
