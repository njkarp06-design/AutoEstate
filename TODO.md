...
- [~] 1H stand up dev buyer instance + end-to-end test. **Instance BUILT and statically verified 2026-07-26; live run not yet done** (needs the Telegram bot token, below). Sub-status:
  ...
  - [ ] **Test-data note:** both `Listing` rows in the dev DB are currently `SOLD` (Ben Gurion, Dizengoff). That exercises the SOLD-honesty path well but not the happy path — an `ACTIVE` listing is needed to test a real factual answer. Either send a fresh listing through the operator WhatsApp bot or seed one.
