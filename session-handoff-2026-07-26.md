...
- **On `main`, clean, nothing in flight.** Every PR opened to date is merged and **no PR is open** — deliberately not enumerated here, since a PR range goes stale the moment the next one merges (including the one that updates this line). Check with `gh pr list`. All merged branches have been deleted locally and on origin. Exactly two branches remain: `main`, and `chore/doc-consistency-checker` — deliberately kept and **intentionally unmerged**, holding the only copy of the closed PR #33 doc-checker script (`.claude/doc-consistency-check.sh`). Do not delete it as "stale": its commits exist nowhere else. It is also **far behind `main` by design** and drifts further with every merge — cherry-pick the script if you ever want it, never merge the branch. (No counts stated, in either direction: this sentence has now gone stale twice by naming one. `git rev-list --count chore/doc-consistency-checker..main` if you actually need the number.)
...
- **Every merged plugin fix is deployed and live** (2026-07-27, #37 and then #43). Merging does not do this — plugins are physical copies inside each profile — so each time they were copied across and the operator gateway restarted. Parity was 5/5 after both, and the running process demonstrably loaded the new code. Nothing is half-deployed. Re-run CLAUDE.md §5's parity recipe rather than trusting this line; it is only true until the next plugin change.
...
| `autoestate` gateway (operator) | running, PID 36868 — the live WhatsApp bot, restarted 2026-07-27 19:58 onto PR #43's plugin fixes (start time verified later than the files, so it loaded them) |
...
| Repo↔live plugin parity (5 plugins) | all 5 match (re-verified after deploying #43's fixes, the most recent plugin change) |
...
3. **Harden the public buyer instance** (items 5–6). It no longer shares the operator's ingestion secret — that was fixed 2026-07-27, and its own credential is rejected by `/api/ingest`. What remains is OS-level isolation (`terminal.backend: local`, no sandboxing) and re-running `hermes security audit` once anything is publicly exposed — ~50 known CVEs are currently unreachable only because nothing is exposed.
...
- **After ANY schema change, restart the `next dev` server on 4127.** The generated Prisma client loads at server *start*, never by hot-reload, so a running server keeps querying the old shape. Symptom: machine routes return **500 where they used to return 401**. This is not cosmetic — while it lasts, the live gateway's sync POSTs fail, so a real WhatsApp turn is lost. Bit us on 2026-07-27 during the credential rename.
- Dev DB: one customer, four listings — 2 `ACTIVE` (Rothschild Boulevard, Neve Tzedek, seeded for testing) and 2 `SOLD` (Ben Gurion, Dizengoff). That customer now has **both** machine credentials set (operator + buyer).
...
Shipped from `fix/inspect-sweep-2026-07-27`. Branch and PR state is deliberately not asserted here — it goes stale the moment the thing merges, including the line saying so. `git log origin/main..` and `gh pr list --state all` are the source of truth.
...
**The four changed plugins were redeployed and the operator gateway restarted the same day** (2026-07-27 19:58, by the owner from their own PowerShell — doing it from an agent session has caused a full outage before). Parity 5/5; PID 6072 → 36868 with a start time later than the newest plugin file, which is the check that actually proves new code was loaded rather than a cache; clean log, both platforms connected, no errors, and the sweep's new inert-plugin warning correctly silent. The buyer profile's two copies are in place but unloaded — its gateway is stopped and its token revoked, so they apply whenever it next starts.
...
`chore/doc-consistency-checker` now genuinely is what the docs say it is: `settings.json` (which restored **both** hooks, including the rejected `UserPromptSubmit` injector) deleted, stale doc hunks reverted, and the script given a real `--force` on-demand mode plus a lock. It stays far behind `main` by design — **cherry-pick the script, never merge the branch.**

### How to resume

> Read session-handoff-2026-07-26.md and continue. Everything is built and merged; verify with `git log origin/main..` and `gh pr list --state all` rather than trusting any branch/PR claim in these docs. The 2026-07-27 sweep's plugin fixes are deployed and live (parity 5/5 — but re-run CLAUDE.md §5's parity recipe rather than trusting this line, since any later plugin change breaks it again). **One thing it left me:** the buyer-channel transport decision now carries a security gate, because the buyer profile's lockdowns are Telegram-only. Beyond that, what's left is productionization, starting with whichever account-level step I authorize — Hetzner + `terraform apply` (TODO item 3) or the Vercel deploy (item 4). Don't start either without me: both cost money and neither account exists yet. The scoped second ingestion secret — previously the top unprompted item — is done: the buyer instance has its own credential and is rejected by `/api/ingest`, and the buyer profile's local `.env` already holds it. If a dev server is running from an older checkout, restart it — the Prisma client loads at start, so it will 500 on every machine route until you do.
